...
export default function TaskFormModal({ members, onSubmit, onClose }: Props) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 rounded-2xl p-6 w-full max-w-md max-h-screen overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-white text-xl font-bold">Nova Tarefa</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {members.length > 0 && (
            <div>
              <label className="text-slate-400 text-sm mb-1 block">
                Responsável <span className="text-slate-500">(opcional)</span>
              </label>
              <select
                value={form.assigneeId}
                onChange={e => setForm({ ...form, assigneeId: e.target.value })}
                className="w-full bg-slate-800 text-white px-4 py-3 rounded-lg outline-none focus:ring-2 focus:ring-blue-500">
                <option value="">Sem responsável (atribuído ao mover)</option>
                {members.map(m => (
                  <option key={m.id} value={m.id}>{m.name} ({m.role})</option>
                ))}
              </select>
              <p className="text-slate-500 text-xs mt-1">
                Se não atribuir, quem mover para "Em Progresso" vira o responsável automaticamente.
              </p>
            </div>
          )}
          <div>
          ...
          {form.deadlineDate && (
            <div>
              ...
              {!form.useTime && (
                <p className="text-slate-500 text-xs mt-1">
                  Sem horário definido, será considerado 23:59 do dia.
                </p>
              )}
            </div>
          )}
